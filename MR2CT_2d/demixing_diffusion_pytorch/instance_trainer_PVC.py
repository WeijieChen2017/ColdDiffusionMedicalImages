...
class instance_trainer_PVC(object):
    def __init__(
        self,
        model,
        data_loader,
    ...
    def generate_xt1_xt2(self, data_1, data_2, device):
        ...
        t_int = torch.randint(1, self.time_steps-1, (1,))
        t_int_next = t_int + 1
        
        # make both t_1 and t_2 of the shape of batch_size
        t = t_int.expand(data_1.shape[0])
        t_next = t_int_next.expand(data_1.shape[0])
        

        alpha = (t_int.float() / self.time_steps)
        alpha_next = (t_int_next.float() / self.time_steps)

        # Explicitly broadcasting alpha_1 and alpha_2
        alpha = alpha.view(-1, 1, 1, 1)

        data_t1 = (1 - alpha) * data_1 + (alpha) * data_2
        data_t2 = (1 - alpha_next) * data_1 + (alpha_next) * data_2
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            for i in range(self.gradient_accumulate_every):
                ...
                data_t1, data_t2, t, t_int = self.generate_xt1_xt2(data_1, data_2, device='cuda')

                data_t2_hat = self.model(data_t1, t)
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                imgs_to_plot = [
                    ...
                    [data_t1, f'data_t1_{int(t_int)}'],
                    [data_t2, f'data_t2_{int(t_int)}'],
